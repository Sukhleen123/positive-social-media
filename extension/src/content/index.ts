...
// Selectors covering all Reddit page types:
// - shreddit feed/comments (web components, 2023+): shreddit-post
// - Old new-Reddit redesign (2018-2023): div[data-testid="post-container"]
// - Search results page: div[data-testid="search-post-with-content-preview"]
const POST_SELECTOR = 'shreddit-post, div[data-testid="post-container"], div[data-testid="search-post-with-content-preview"]';
...
function getElementId(el: Element): string {
  ...
  // Search results page: title anchor has id="search-post-title-t3_xxx"
  const searchTitleEl = el.querySelector('a[id^="search-post-title-"]');
  if (searchTitleEl) {
    const rawId = searchTitleEl.getAttribute('id')!.replace('search-post-title-', '');
    return 'psm-' + rawId;
  }
...
function getPostText(el: Element): string {
  ...
  // 2. Search results page: dedicated title + snippet anchors
  const searchTitle = el.querySelector('a[data-testid="post-title-text"]');
  if (searchTitle?.textContent?.trim()) {
    const snippet = el.querySelector('a[data-testid="post-title"]')?.textContent?.trim() ?? '';
    return (searchTitle.textContent.trim() + (snippet ? ' ' + snippet : '')).slice(0, 500);
  }

  // 3. Shreddit: title anchor with id="post-title-{shortId}" (confirmed in Reddit DOM)
